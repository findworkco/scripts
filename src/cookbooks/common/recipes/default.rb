...
service "ssh" do
  ...
  # DEV: We use `Dir.exists?` over `ENV` since we restrict our variable passing in `findwork.co.sh`
  provider(Dir.exists?("/pipeline") ? Chef::Provider::Service::Init : Chef::Provider::Service::Upstart)
